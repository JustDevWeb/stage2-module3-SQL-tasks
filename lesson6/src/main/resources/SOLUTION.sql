...
SELECT s.id,s.name,s.birthday,s.groupnumber FROM STUDENT s LEFT JOIN PAYMENT pt on s.id = pt.student_id WHERE pt.type_id = ( select id from PAYMENTTYPE where name = 'WEEKLY');
